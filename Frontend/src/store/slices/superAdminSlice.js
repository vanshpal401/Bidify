...
const superAdminSlice = createSlice({
  ...
  reducers: {
    ...
    alluserDataRequest(state,action){
        state.loading=true;
    },
    ...
    alluserDataFailed(state,action){
        state.loading=false;
        state.allUsers=[];
    },

    getUserByidRequest(state,action){
        state.loading=true;
    },
...
export const getAllUsers = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      "http://localhost:5000/api/v1/superadmin/user/getall",
      { withCredentials: true }
    );
...
export const getSinglePaymentProofDetail = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `http://localhost:5000/api/v1/superadmin/paymentproof/get/${id}`,
      { withCredentials: true }
    );
    dispatch(
...
export const fetchAllUserData = () => async (dispatch) => {
    dispatch(superAdminSlice.actions.alluserDataRequest());
    try {
      const response = await axios.get(
        "http://localhost:5000/api/v1/superadmin/user/allusers",
        { withCredentials: true }
      );
      dispatch(superAdminSlice.actions.alluserDataSuccess(response.data.user));
    } catch (error) {
      dispatch(superAdminSlice.actions.alluserDataFailed());
      console.error(error.response.data.message);
    }
  };

export const getUserDetails=(id)=>async(dispatch)=>{
    dispatch(superAdminSlice.actions.getUserByidRequest())
    try {
        const response=await axios.get(`http://localhost:5000/api/v1/superadmin/user/${id}`,{withCredentials:true,});
        dispatch(superAdminSlice.actions.getUserByidSuccess(response.data));
    } catch (error) {
        dispatch(superAdminSlice.actions.getUserByidFailed());
      console.error(error.response.data.message);
    }
...
export const deleteUser=(id)=>async(dispatch)=>{
  dispatch(superAdminSlice.actions.deleteUserRequest());
  try {
    const response = await axios.delete(
      `http://localhost:5000/api/v1/superadmin/user/delete/${id}`,
      { withCredentials: true }
    );
    dispatch(superAdminSlice.actions.deleteUserSuccess());
    toast.success(response.data.message);
    dispatch(getAllAuctionItems());
  } catch (error) {
    dispatch(superAdminSlice.actions.deleteUserFailed());
    console.error(error.response.data.message);
    toast.error(error.response.data.message);
  }
}
